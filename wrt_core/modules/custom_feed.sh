...
install_custom_feed() {
    ...
    if ! fix_emmc_health_luci_js_deps "$custom_feed_dir/luci-app-emmc-health"; then
        ...
        # 同步 js 版 luci-app-tailscale-community（兼容 js LuCI，无 ACL 问题）
    local tailscale_community_tmp=$(mktemp -d)
    echo "正在从 Tokisaki-Galaxy/luci-app-tailscale-community 同步 js 版 tailscale..."
    if git_retry clone --depth 1 "https://github.com/Tokisaki-Galaxy/luci-app-tailscale-community.git" "$tailscale_community_tmp"; then
        rm -rf "$custom_feed_dir/luci-app-tailscale"
        mv "$tailscale_community_tmp" "$custom_feed_dir/luci-app-tailscale"
        echo "js 版 luci-app-tailscale 同步完成"
    else
        echo "错误：同步 luci-app-tailscale-community 失败" >&2
        rm -rf "$tailscale_community_tmp"
        rm -rf "$custom_feed_dir"
        return 1
    fi
